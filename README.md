...
1) Реализация контроллеров в модуле ***catalog/views.py***:
   ...
   - функциональный контроллер `home_page(request)` - контроллер для отображения домашней страницы (***home.html***) с пагинацией:
   ...
   - функциональный контроллер `add_product_page(request)` - контроллер для отображения страницы с формой (***add_your_product.html***), которая позволяет пользователю добавлять новые товары в БД:
     - Обрабатываемые запросы:
       - **GET-запрос**: контроллер рендерит шаблон add_your_product.html;
       - **POST-запрос**: контроллер вызываю форму для добавления пользователем нового товара (форма создана в forms.py).
...
Форма `ProductForm(forms.ModelForm)` - Django-форма для удобной валидации и работы с пользовательским вводом.
