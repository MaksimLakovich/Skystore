...
1. Реализованы следующие HTML-страницы:
   - `шаблонные страницы:`
     ...
     - ***menu.html***: подшаблон, в котором описано главное меню:
       ...
       - блок с кнопками аутентификации ("Регистрация", "Войти", "Выйти") и логикой их отображения в зависимости от текущего состояния пользователя (авторизован или нет).
...
1) В приложении используется определение и обработка представлений с помощью классов (CBV: class-based view):
   ...
   - `class RegisterView(FormView)` - представление для отображения страницы регистрации нового пользователя (***register.html***):
     - Обрабатываемые запросы:
...
1) Реализация модели ***UserCustomer***, которая представляет представляет пользователя магазина:
