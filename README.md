...
## <a id="title2">2. Описание HTML-страниц для приложения Catalog</a>
1. Реализованы следующие HTML-страницы (`templates/catalog/`):
...
2. Часть страниц реализована по шаблонам, которые были явно зафиксированы в БТ (шаблоны размещены по адресу: `static/html_patterns/`):
   - ***static/html_patterns/Home_page.html***: содержит HTML-шаблон для страницы "Главная", согласно прототипа из ТЗ:
   ![Прототип для страницы "Главная"](static/html_patterns/Home_page.png)
    
   - ***static/html_patterns/Contacts_page.html***: содержит HTML-шаблон для страницы "Контакты", согласно прототипа из ТЗ:
   ![Прототип для страницы "Контакты"](static/html_patterns/Contacts_page.png)
   
   - Страницы, по которым не было предоставлено явных шаблонов в БТ, реализовывались на основе стилизации предыдущих шаблонов.
...
1) Реализация контроллеров в модуле ***catalog/views.py***:
   ...
   - функциональный контроллер `home_page(request)` - контроллер для отображения домашней страницы (***home.html***):
   ...
   - функциональный контроллер `contacts_page(request)` - контроллер для отображения страницы "Контакты" (***contacts.html***).
     - Обрабатываемые запросы:
   ...
   - функциональный контроллер `product_detail(request, pk)` - контроллер для отображения страницы с подробной информацией о продукте (***product.html***):
     - Обрабатываемые запросы:
